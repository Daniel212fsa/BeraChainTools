...
def swap_usdc(account, bera):
    ...
    for i in range(10):
        try:
            ...
            if usdc_balance > 0:
                logger.success(f"{account_address},已兑换过usdc!")
                break
            ...
            logger.debug(
                f'{account_address},usdc_balance:{usdc_balance / 10 ** 18},usdc_balance_new:{usdc_balance_new / 10 ** 18}')
            if result:
                logger.success(f'{account_address},bex 使用bera交换usdc成功,{result}')
                break
            else:
                logger.error(f'{account_address},bex 使用bera交换usdc失败,{result}')
        except Exception as e:
            logger.error(f'{account_address},bex 使用bera交换usdc失败,{e}')
...
def swap_weth(account, bera):
    ...
    for j in range(10):
        try:
            ...
            if weth_balance > 0:
                logger.success(f"{account_address},已兑换过weth!")
                break
            logger.debug(f'{account_address},weth_balance:{weth_balance}')
            random_amount = round(random.uniform(0.10, 0.20), 2)
            ...
            logger.debug(
                f'{account_address},weth_balance:{weth_balance / 10 ** 18},weth_balance_new:{weth_balance_new / 10 ** 18}')
            if result:
                logger.success(f'{account_address},bex 使用bera交换weth成功,{result}')
                break
            else:
                logger.error(f'{account_address},bex 使用bera交换weth失败,{result}')
        except Exception as e:
            logger.error(f'{account_address},bex 使用bera交换weth失败,{e}')
                ...
                for k in range(10):
                    try:
                        ...
                        result = bera.bex_add_liquidity(int(usdc_balance * random_amount), usdc_pool_liquidity_address,
                                                        usdc_address)
                        if result:
                            logger.success(f'{account_address},增加 usdc 流动性成功,{result}')
                            break
                        else:
                            logger.error(f'{account_address},增加 usdc 流动性失败,{result}')
                    except Exception as e:
                        logger.error(f'{account_address},增加 usdc 流动性失败,{e}')
                break
            else:
                logger.error(f'{account_address},授权失败!')
        except Exception as e:
            logger.error(f'{account_address},增加 usdc 流动性失败,{e}')
...
def add_liquidity_weth(account, bera):
    ...
    for k in range(10):
        try:
            weth_balance = bera.weth_contract.functions.balanceOf(account.address).call()
            if weth_balance == 0:
                logger.error(f'{account_address},没有weth,自动跳过!')
                break
                ...
                for t in range(10):
                    try:
                        ...
                        result = bera.bex_add_liquidity(int(weth_balance * random_amount), weth_pool_liquidity_address,
                                                        weth_address)
                        if result:
                            logger.success(f'{account_address},增加 weth 流动性成功,{result}')
                            break
                        else:
                            logger.error(f'{account_address},增加 weth 流动性失败！！！,{result}')
                    except Exception as e:
                        logger.error(f'{account_address},增加 weth 流动性失败！！！,{e}')
                break
            else:
                logger.error(f'{account_address},授权失败!')
        except Exception as e:
...
def bex_interacte(private_key, rpc_url):
    ...
    account_address = account.address
    logger.debug(f'{account_address},开始Bex 交互')
    ...
    logger.success(f'{account_address},Bex 交互结束')
    logger.debug('-------------------------------------------------------------------------------------')
