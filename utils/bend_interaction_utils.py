...
def bend_interacte(private_key, rpc_url, index, try_times):
    for _ in range(try_times):
        if bend_deposit_honey(private_key, rpc_url, index, try_times):
            break
    for _ in range(try_times):
        if bend_deposit_weth(private_key, rpc_url, index, try_times):
            break
    for _ in range(try_times):
        if bend_borrow(private_key, rpc_url, index, try_times):
            break
    for _ in range(try_times):
        if bend_repay(private_key, rpc_url, index, try_times):
            break


def bend_deposit_honey(private_key, rpc_url, index, try_times):
    try:
        account = Account.from_key(private_key)
        bera = BeraChainTools(private_key=account.key, rpc_url=rpc_url)
        logger.debug(f'第{index}次交互,开始存款honey')
...
def bend_deposit_weth(private_key, rpc_url, index, try_times):
    try:
        account = Account.from_key(private_key)
        bera = BeraChainTools(private_key=account.key, rpc_url=rpc_url)
        logger.debug(f'第{index}次交互,开始存款weth')
...
def bend_borrow(private_key, rpc_url, index, try_times):
    try:
        account = Account.from_key(private_key)
        bera = BeraChainTools(private_key=account.key, rpc_url=rpc_url)
        logger.debug(f'第{index}次交互,开始借款')
...
def bend_repay(private_key, rpc_url, index, try_times):
    try:
        account = Account.from_key(private_key)
        bera = BeraChainTools(private_key=account.key, rpc_url=rpc_url)
        logger.debug(f'第{index}次交互,开始还款')
        vdbalance = bera.vdhoney_contract.functions.balanceOf(account.address).call()
        if vdbalance > 0:
            random_amount = round(random.uniform(0.01, 0.08), 2)
